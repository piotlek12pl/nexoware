...
local library = {
    ...
    themes = {
        Default = {
            ["Accent"] = fromRGB(245, 158, 11),
            ["Window Background"] = fromRGB(20, 20, 22),
            ["Window Border"] = fromRGB(55, 55, 55),
            ["Black Border"] = fromRGB(10, 10, 10),
            ["Text"] = fromRGB(220, 220, 220),
            ["Disabled Text"] = fromRGB(110, 110, 110),
            ["Tab Background"] = fromRGB(18, 18, 20),
            ["Tab Border"] = fromRGB(45, 45, 45),
            ["Section Background"] = fromRGB(20, 20, 22),
            ["Section Border"] = fromRGB(45, 45, 45),
            ["Object Background"] = fromRGB(18, 18, 20),
            ["Object Border"] = fromRGB(45, 45, 45),
            ["Dropdown Option Background"] = fromRGB(14, 14, 16)
        }
    }
}
        ...
        if focused then
            if input.UserInputType == Enum.UserInputType.Keyboard then
                ...
                elseif input.KeyCode ~= Enum.KeyCode.Backspace then
                    ...
                    local base_str = services.UserInputService:GetStringForKeyCode(input.KeyCode)
                    local key_str = base_str

                    if key_str ~= "" then
                        local shift_held = input:IsModifierKeyDown(Enum.ModifierKey.Shift)
                        if shift_held then
                            -- Mapa znakow specjalnych (US QWERTY) gdy trzymany Shift
                            local shift_map = {
                                ["1"]="!", ["2"]="@", ["3"]="#", ["4"]="$", ["5"]="%",
                                ["6"]="^", ["7"]="&", ["8"]="*", ["9"]="(", ["0"]=")",
                                ["-"]="_", ["="]="+", ["["]="{", ["]"]="}", ["\\"]="|",
                            ...
                            if shift_map[key_str] then
                                key_str = shift_map[key_str]
                            else
                                key_str = key_str:upper()
                            end
                        else
                            -- bez shifta: litery male (cyfry/znaki zostaja jakie sa)
...
function components.colorpicker(holder, options, zindex)
    ...
    local icon = holder.main:Create("Square", {
        ...
        Position = newUDim2(1, -(18 + holder.colorpickers * (holder.colorpickers * 24) + (holder.keybind and holder.keybind.AbsoluteSize.X + 12 or 0)), 0, 2),
        ZIndex = zindex,
        OutlineTheme = "Object Border"
    })
...
function library:SaveConfig(name)
    if typeof(name) == "string" and name:find("%S+") then
        ...
        -- NexoHub: zapis stanu skinow/unlockow bezposrednio w configu Exodus
        if genv._GetSkinState then
            local ok, skinState = pcall(genv._GetSkinState)
            if ok and type(skinState) == "table" then
                config["__nexo_skins"] = skinState
            end
        end
...
function library:LoadConfig(name)
    if typeof(name) == "string" then
        ...
        if file then
            ...
            -- NexoHub: odtworzenie stanu skinow/unlockow z configu Exodus
            if genv._LoadSkinState and config["__nexo_skins"] then
                spawn(function()
                    genv._LoadSkinState(config["__nexo_skins"])
                end)
            end

            -- Wywołaj callback po załadowaniu configu (używany przez auto-unlock)
            if genv._OnConfigLoaded then
                spawn(function()
                    wait(0.5)
                    genv._OnConfigLoaded(self.flags)
                end)
            end
...
function library:Playerlist(max_players)
    ...
    local function create_card(plr)
        if not player_data[plr].image then
            ...
            spawn(function()
                local ok, thumbnail_data = pcall(function()
                    return services.HttpService:JSONDecode(game:HttpGet(("https://thumbnails.roblox.com/v1/users/avatar?userIds=%s&size=150x150&format=Png"):format(plr.UserId)))
                end)
                if not ok or not thumbnail_data or not thumbnail_data.data or not thumbnail_data.data[1] then return end
                local imageUrl = thumbnail_data.data[1].imageUrl
                if not imageUrl then return end

                local image_data
                if syn and syn.request then
                    local ok2, resp = pcall(function() return syn.request{Url = imageUrl, Method = "GET"}.Body end)
                    if ok2 then image_data = resp end
                elseif request then
                    local ok2, resp = pcall(function() return request{Url = imageUrl, Method = "GET"}.Body end)
                    if ok2 then image_data = resp end
                elseif http_request then
                    local ok2, resp = pcall(function() return http_request{Url = imageUrl, Method = "GET"}.Body end)
                    if ok2 then image_data = resp end
                end

                if image_data then
                    player_data[plr].image = image_data
                    if current_player == plr then
                        headshot.Data = image_data
                    end
                end
            end)
        else
